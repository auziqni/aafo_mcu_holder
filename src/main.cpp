...
bool sessionStartFromWeb = false;
...
void writeFirebase()
{
  if (Firebase.ready())
  {
    writeToFirebase("/dataNew/beratBelakang", String(fsr2value), "float");
    writeToFirebase("/dataNew/beratDepan", String(fsr1value), "float");
    writeToFirebase("/dataNew/sudut", String(pitch), "float");
  }
  else
  {
...
void readFirebaseStart()
{
  if (Firebase.ready())
  {
    Firebase.getBool(fbdo, "/dataNew/sessionStart") && (sessionStartFromWeb = fbdo.boolData());
    Serial.print("sessionStartFromWeb = ");
    Serial.println(sessionStartFromWeb);
  }
  else
  {
    Serial.println("Firebase not ready");
...
void writeFirebaseStart()
{
  if (Firebase.ready())
  {
    writeToFirebase("/dataNew/sessionStart", "false", "bool");
  }
  else
  {
    Serial.println("Firebase not ready");
...
void loop()
{
  ...
  while (sessionStartFromWeb)
  {
    ...
    for (int i = 0; i < sizeof(poss) / sizeof(poss[0]); i++) // loop akan berjalan dari i = 0 hingga i = 8 (9 iterasi), mengakses setiap elemen dalam array poss.
    {
      ...
      {
        // --------------------------------- Read the FSR values
        fsr1value = abs(analogRead(FSR_1_PIN) * -0.799);
        fsr2value = abs(analogRead(FSR_2_PIN) * -0.729);
        ...
        {
          timersecond = millis();
          Serial.print(sizeof(poss) / sizeof(poss[0]));
          Serial.print("\t,data i =");
          Serial.println(i);
          writeFirebase();
          delay(100);
          readFirebaseStart();
          if (!sessionStartFromWeb)
          {
            break;
          }
          delay(100);
